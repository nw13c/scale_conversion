...
# we need to find a way to apply the function to a list of dataframe, so lappy is used here. The purpose of aggregrate() is to help us
# find all possible combinations between standard score (ss) and age (age_range).
...
library(reshape2)
# use the library(reshape2) to transform the long data into wide data and it can put the row and column in the right order
...
# write a function to find the range from a given data (it's tested on the sigle data called scale. If you are interested, please 
#try it by yourself.)
